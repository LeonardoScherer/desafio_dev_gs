<div {{ $attributes->merge(['class' => 'md:grid md:grid-cols-3 md:gap-6']) }}>
    <x-section-title>
        <x-slot name="title">Bulk User Registration</x-slot>
        <x-slot name="description">Insert a spreadsheet (XLSX or CSV) to register users in bulk</x-slot>
    </x-section-title>
    <div class="mt-5 md:mt-0 md:col-span-2">
        <form method="POST" action="{{ route('store.spreadsheet-users') }}" enctype="multipart/form-data">
            ...
            <div class="px-4 py-5 bg-white dark:bg-gray-800 sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                <label for="file" class="block text-white font-bold ml-3 ">Upload file</label>
